...
define(["jquery", "U", "rest", "uiEngine", "cirm", "businessobject"], function($, U, rest, ui, cirm, bo)   {
    ...
    function startInquiry(iri)
    {
        ...
        $.getJSON(urlBase + '/op/new/' + classname, function (data) {
                ...
                currentRequest.runWorkflowWizard(parentElement, function() {
                        mainUI();
                });                
/*                $.getJSON(self.urlBase + '/op/workflow/' + classname + '/' + data.boid, function(x) {
                self.currentRequest = x;
                var comp = uiEngine.makeDomTree(x);
                self.setui(comp);
            });
