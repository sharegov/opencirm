...
public class UploadToCloudServerResource extends ServerResource
{
	...
	public synchronized String getUploadServiceUrl()
	{
		...
		return ind == null ? null
				: OWL.dataProperty(ind, "hasUrl").getLiteral();
	}
	...
	public Representation upload(Representation entity)
	{
		...
		if (entity == null && org.restlet.data.MediaType.MULTIPART_FORM_DATA
				.equals(entity.getMediaType(), true) == false)
		{
			return rep;
		}
		...
		try
		{
			...
			while (fit.hasNext())
			{
				...
				if (stream.getFieldName().equals("uploadAttachment"))
				{
					...
					byte[] file = GenUtils
							.getBytesFromStream(stream.openStream(), true);
					...
					StringRequestEntity requestEntity = new StringRequestEntity(
							request.toString(), "application/json", "UTF-8");
					...
					else
					{
						...
						json.set("ok", true);
						json.set("key",
								result.at("value").at("key").asString());
						json.set("url",
								result.at("value").at("url").asString());

					}
					rep = new StringRepresentation(json.toString(),
							(org.restlet.data.MediaType) org.restlet.data.MediaType.TEXT_HTML);
					break;
